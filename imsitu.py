...
class imSituTensorEvaluation():
    def __init__(self, topk, nref, image_group=None):
        self.score_cards = {}
        self.topk = topk
        self.nref = nref
        self.image_group = {} if image_group is None else image_group
    ...
    def get_average_results(self, groups=None):
...
class imSituVerbRoleNounEncoder:
    ...
    def __init__(self, dataset):
        ...
        for (image, annotation) in dataset.items():
            ...
            for frame in annotation["frames"]:
                for (r, n) in frame.items():
                    ...
                    nid = self.n_id[n]
                    if nid not in self.r_n[rid]:
                        self.r_n[rid].append(nid)
        ...
        for (r, rid) in self.r_id.items():
            self.r_n[rid] = sorted(self.r_n[rid])
